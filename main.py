...
def fill_in_file(all_tags,doc):  
    count = 0    
    for tag in all_tags: 
        bs_tag = BeautifulSoup(tag,features="html.parser")
        if  'mw-file-description' in tag:
            count += 1
            img_link = bs_tag.find('img').get('src')
            ...
            run = p.add_run()
            run.add_picture(Converts_an_image_to_jpg(f'img/picture{count}.jpg'))
            p.alignment =  WD_ALIGN_PARAGRAPH.CENTER
            delete_everything_in_folder('img')
        if '<p>' in tag:
            p = doc.add_paragraph(bs_tag.text)
            p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            continue
        for a in range(1,5):
            if f'<h{a}' in tag:
                head = doc.add_heading(bs_tag.text, level=a)
...
def Converts_an_image_to_jpg(image_path):
...
if __name__ == '__main__':
    ...
    title_text =  soup.find('h1', class_="firstHeading mw-first-heading").text
    
    styles = setting_styles(doc)

    head = doc.add_heading(title_text, level =1)
    head.alignment = WD_ALIGN_PARAGRAPH.CENTER
